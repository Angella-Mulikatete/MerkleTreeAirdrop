...
describe("merkleAidrop", function () {
  ...
  async function deploymerkleAirdrop() {
    ...
    const merkle_Airdrop = await merkleAirdrop.deploy(token);

    return { merkle_Airdrop, owner, otherAccount, token };
  }

  describe("Deployment", function () {
    it("Should check if owner is correct", async function () {
      const { merkle_Airdrop, owner } = await loadFixture(deploymerkleAirdrop);

      expect(await saveErc20.owner()).to.equal(owner);
    });

    it("Should check if tokenAddress is correctly set", async function () {
      const { saveErc20, owner, token } = await loadFixture(deploymerkleAirdrop);
  ...
  describe("Deposit", function () {
    it("Should deposit successfully", async function () {
      const { saveErc20, owner, otherAccount, token } = await loadFixture(deploymerkleAirdrop);
    ...
    it("Should emit an event after successful deposit", async function () {
      const { saveErc20, otherAccount, token } = await loadFixture(deploymerkleAirdrop);
